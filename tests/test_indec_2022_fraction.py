...
def test_fraction_uses_cod_indec_as_authority_and_preserves_source_codes(tmp_path):
    ...
    chaco = normalized.loc[normalized["native_id"] == "2214020"].iloc[0]
    ...
    source = tmp_path / "fraction_fixture.geojson"
...
def test_fraction_retains_supporting_code_disagreement_as_warning():
    rows = standard_rows()
    rows[0]["cde"] = "999"
...
def test_fraction_retains_unclassified_zero_without_no_data_claim():
    rows = standard_rows()
    rows[0]["cfn"] = 0
    rows[0]["cod_indec"] = 200100
    normalized, audit = normalize_source(make_frame(rows), load_config())
    row = normalized.loc[normalized["native_id"] == "0200100"].iloc[0]
...
def test_fraction_retains_invalid_polygon_as_nonanalytical_warning():
    frame = make_frame(standard_rows())
    frame.at[0, "geometry"] = Polygon([(0, 0), (1, 1), (0, 1), (1, 0), (0, 0)])
    normalized, audit = normalize_source(frame, load_config())
    invalid = normalized.loc[normalized["native_id"] == "0200101"].iloc[0]
...
def test_fraction_preserves_source_driver_warning():
    _, audit = normalize_source(
        make_frame(standard_rows()),
        load_config(),
        driver_warnings=["fixture polygon rings with invalid winding order"],
    )
    assert "noncanonical_ring_winding" in warning_codes(audit)


def test_fraction_stops_on_non_areal_geometry():
    frame = make_frame(standard_rows())
...
def test_fraction_rejects_duplicate_native_identity():
    rows = standard_rows()
...
def test_fraction_rejects_unbounded_source_department_code():
    rows = standard_rows()
    rows[0]["cde"] = "123456"
