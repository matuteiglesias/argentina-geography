...
def download_source(destination: Path, config: dict) -> Path:
    ...
    if destination.stat().st_size == 0:
        raise ValueError("CEUR 2010 V2025-1 download returned an empty source file")
    if destination.read_bytes()[:2] != b"PK":
        raise ValueError("CEUR 2010 V2025-1 response is not a ZIP archive; source endpoint may have changed")
    return destination
...
def _read_single_layer(source_path: Path) -> tuple[gpd.GeoDataFrame, list[dict]]:
    ...
    if len(layers) != 1:
        raise ValueError(
            "CEUR 2010 V2025-1 probe expected one vector layer; "
            f"observed {len(layers)}: {records}"
        )
    frame = gpd.read_file(
        source, layer=layers.iloc[0]["name"], engine="pyogrio", use_arrow=True
    )
...
def _profile_field(frame: gpd.GeoDataFrame, field: str) -> dict:
    ...
    return {
        ...
        "sample_values": values.drop_duplicates().head(20).tolist(),
    }
...
def profile_source(source_path: Path, output: Path, config_path: Path = DEFAULT_CONFIG) -> dict:
    ...
    preview = frame[fields].head(15).copy().astype(object).where(lambda x: x.notna(), "<NA>")
    evidence = {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Characterize CEUR Census 2010 radio V2025-1 before normalization."
    )
