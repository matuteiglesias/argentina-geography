...
DEFAULT_CONFIG = ROOT / "config/sources/ign_administrative_province.json"
...
DISPLAY_PROPERTY_FIELDS = ("geography_id", "geo_uid", "native_id", "FNA", "GNA", "NAM")
...
def download_source(destination: Path, config: dict) -> Path:
    ...
    if destination.stat().st_size == 0:
        raise ValueError("IGN province archive download returned an empty file")
    return destination
...
def normalize_source(
    ...
    *,
    ...
    if missing_fields:
        raise ValueError(f"IGN province source is missing required fields: {missing_fields}")
    if frame.crs is None:
        raise ValueError("IGN province source requires an explicit CRS")
    expected_crs = config.get("expected_crs", "EPSG:4326")
    if frame.crs.to_string() != expected_crs:
        raise ValueError(
            f"IGN province source CRS drifted: expected {expected_crs}, found {frame.crs.to_string()}"
        )
    ...
    if result[identity_field].duplicated().any():
        ...
        raise ValueError(
            f"IGN province native IDs must be unique; duplicates include {duplicates[:10]}"
        )

    expected_ids = tuple(config["expected_native_ids"])
    actual_ids = tuple(sorted(result[identity_field].tolist()))
    if actual_ids != tuple(sorted(expected_ids)):
        raise ValueError(
            "IGN province native ID set drifted: "
            f"expected {sorted(expected_ids)}, found {list(actual_ids)}"
        )

    secondary_id = config["secondary_source_identifier"]
    if result[secondary_id].isna().any() or result[secondary_id].duplicated().any():
        raise ValueError(f"IGN province {secondary_id} must be non-missing and unique")
    ...
    if object_values != {config["expected_object_value"]}:
        raise ValueError(
            f"IGN province {object_field} field drifted: expected "
    ...
    if gna_values != expected_gna:
        raise ValueError(
            f"IGN province {gna_field} vocabulary drifted: expected {sorted(expected_gna)}, "
    ...
    if missing_geometry or empty_geometry or non_areal_geometry:
        raise ValueError(
            "IGN province geography has unusable source geometry: "
    ...
    if invalid_geometry:
        warnings.append(
            _warning(
                ...
                "Source-invalid IGN province polygons are retained without repair and excluded "
    ...
    if len(result) != expected_count:
        raise ValueError(
            f"IGN province feature count drifted: expected {expected_count}, found {len(result)}"
        )

    snapshot_token = source_sha256[:12]
    result["native_id"] = result[identity_field]
    result["geography_id"] = result["native_id"]
    result["geo_uid"] = (
        f"ign:administrative:province:{snapshot_token}:" + result["native_id"].astype(str)
    )
    ...
    result = result[columns].sort_values("geography_id", ignore_index=True)
    ...
    audit = {
        ...
        "atlas_fixture_id_compatibility": actual_ids == tuple(sorted(expected_ids)),
        "native_id_values": list(actual_ids),
...
def _verify_source_snapshot(source_path: Path, config: dict) -> tuple[str, int]:
    ...
    if source_sha256 != expected_sha:
        raise ValueError(
            f"IGN province source snapshot SHA-256 drift: expected {expected_sha}, "
            f"found {source_sha256}"
        )
    if source_size != expected_size:
        raise ValueError(
            f"IGN province source snapshot size drift: expected {expected_size}, "
            f"found {source_size}"
        )
...
def materialize_from_source(
    ...
    if expected_content and content_sha256 != expected_content:
        raise ValueError(
            "IGN province normalized geography content drift: "
            f"expected {expected_content}, found {content_sha256}"
        )

    display_path = output / "geography.geojson"
    _write_display_geojson(normalized, display_path)
    ...
    qa_result = QAResult(
        check_id="ign_province_source_contract",
        state=qa_state,
        message=(
            "Exact IGN province archive is stageable with explicit geometry warnings."
            if qa_state == "YELLOW"
            else "Exact IGN province archive satisfies the 24-jurisdiction geography boundary."
        ),
        metrics={
            "feature_count": audit["feature_count"],
            "unique_native_id_count": audit["unique_native_id_count"],
            "atlas_fixture_id_compatibility": audit["atlas_fixture_id_compatibility"],
    ...
    run = RunManifest(
        run_id=f"ign-province:{source_sha256[:16]}",
        ...
        parameters={
            ...
            "identity_field": config["identity_field"],
            "display_identity_field": config["display_identity_field"],
            "display_identity_rule": "geography_id = IN1 = native_id",
            "geometry_repair_applied": False,
            "geometry_clip_applied": False,
            "geometry_dissolve_applied": False,
    ...
    manifest = {
        ...
        "display_derivative": {
            ...
            "feature_id_rule": "geography_id = IN1 = native_id",
...
def acquire_and_materialize(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="arggeo-ign-province-") as temporary:
        source_path = Path(temporary) / "ign_provincia.zip"
...
def verify_release(output: Path) -> None:
    ...
    if dataset["dataset_id"] != "arggeo.ign.administrative.province":
        raise ValueError("IGN province release has unexpected dataset_id")
    if len(frame) != manifest["row_count"] or len(frame) != 24:
        raise ValueError("IGN province release must contain exactly 24 rows")
    if sha256_file(output / manifest["artifacts"]["geography"]) != dataset["content_sha256"]:
        raise ValueError("IGN province geography content hash mismatch")
    if frame["geo_uid"].isna().any() or frame["geo_uid"].duplicated().any():
        raise ValueError("IGN province geo_uid must be non-missing and unique")
    if frame["native_id"].isna().any() or frame["native_id"].duplicated().any():
        raise ValueError("IGN province native_id must be non-missing and unique")
    if frame["native_id"].ne(frame["IN1"]).any():
        raise ValueError("IGN province native_id must preserve source IN1")
    if frame["geography_id"].ne(frame["IN1"]).any():
        raise ValueError("IGN province geography_id must equal source IN1")
    if frame["OBJECTID"].isna().any() or frame["OBJECTID"].duplicated().any():
        raise ValueError("IGN province source OBJECTID must be non-missing and unique")
    if frame.geometry.isna().any() or frame.geometry.is_empty.any():
        raise ValueError("IGN province release contains missing or empty geometry")
    non_areal = ~frame.geometry.geom_type.isin(["Polygon", "MultiPolygon"])
    if non_areal.any():
        raise ValueError("IGN province release contains non-areal geometry")
    ...
    if (~(analytical | source_invalid)).any():
        raise ValueError("IGN province release has unsupported geometry_role")
    if (~frame.loc[analytical].geometry.is_valid).any():
        raise ValueError("IGN province analytical rows must contain valid geometry")
    if frame.loc[source_invalid].geometry.is_valid.any():
        raise ValueError("IGN province source_invalid rows must correspond to invalid geometry")
    if frame.loc[analytical, "geometry_valid"].ne(True).any():
        raise ValueError("IGN province analytical geometry_valid flags are inconsistent")
    if frame.loc[source_invalid, "geometry_valid"].ne(False).any():
        raise ValueError("IGN province source-invalid geometry_valid flags are inconsistent")

    config = load_config()
    expected_ids = set(config["expected_native_ids"])
    if set(frame["geography_id"]) != expected_ids:
        raise ValueError("IGN province geography_id set is not fixture-compatible")

    display_path = output / manifest["artifacts"]["display_geojson"]
    display = json.loads(display_path.read_text(encoding="utf-8"))
    if display.get("type") != "FeatureCollection" or len(display.get("features", [])) != 24:
        raise ValueError("IGN province display GeoJSON must contain exactly 24 features")
    display_ids = {feature.get("id") for feature in display["features"]}
    property_ids = {
        feature.get("properties", {}).get("geography_id") for feature in display["features"]
    }
    if display_ids != expected_ids or property_ids != expected_ids:
        raise ValueError("IGN province display GeoJSON feature IDs are not fixture-compatible")
    allowed_properties = set(DISPLAY_PROPERTY_FIELDS)
    for feature in display["features"]:
        if set(feature.get("properties", {})) != allowed_properties:
            raise ValueError("IGN province display GeoJSON has unexpected properties")
    if sha256_file(display_path) != manifest["display_derivative"]["content_sha256"]:
        raise ValueError("IGN province display GeoJSON content hash mismatch")
    ...
    if len(catalog) != 1 or catalog.iloc[0]["dataset_id"] != dataset["dataset_id"]:
        raise ValueError("IGN province catalog does not identify the materialized release")
    if catalog.iloc[0]["level"] != "province":
        raise ValueError("IGN province catalog level must be province")
    if catalog.iloc[0]["display_identity_field"] != "geography_id":
        raise ValueError("IGN province catalog must expose geography_id for display transport")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Materialize or verify the exact IGN province archive Geography Release."
    )
